...
def get_econ(table):
    '''
    Adds up every players econ on the score board.
    Returns the sum
    '''
...
def end_buy_phase(page):
    '''
    Drags the matchs slider to the end of buy phase
    '''
...
def get_match_data(url, output_path):
    '''
    ...
    with sync_playwright() as p:  
        ...
        try:
            ...
            page.wait_for_selector('div.c-nav-match', state="attached", timeout=10000)
            page.wait_for_selector('div.team-winner', state="attached", timeout=10000)
            page.wait_for_selector('a.menu-link', state="attached", timeout=10000)
