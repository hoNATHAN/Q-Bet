...
class Round(
    BaseModel
):  # we can change the fields, i just put down what made sense to me
    team_a_econ: float = Field(..., gt=0, description="Team A economy")
    team_b_econ: float = Field(..., gt=0, description="Team B economy")
    team_a_score: int = Field(
        ..., gt=0, description="Team A score"
    )  # can combine these two into a single field
    ...
    team_a_kills_last_round: int = Field(
        ..., gt=0, description="Team A kills last round"
    )
    team_b_kills_last_round: int = Field(
        ..., gt=0, description="Team B kills last round"
    )
    team_a_assists_last_round: int = Field(
        ..., gt=0, description="Team A assists last round"
    )
    team_b_assists_last_round: int = Field(
        ..., gt=0, description="Team B assists last round"
    )
