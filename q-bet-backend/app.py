...
async def get_bets():
    ...
    return [BetOut(**{**bet, "id": str(bet["_id"])}) for bet in bets]
...
async def get_games():
    ...
    return [GameOut(**{**game, "id": str(game["_id"])}) for game in games]
...
async def create_game_round(game_id: str, round: Round):
    ...
    result = await app.mongodb["games"].update_one(
        {"_id": ObjectId(game_id)}, {"$push": {"rounds": doc}}
    )
...
async def get_game_rounds(game_id: str):
    ...
    if not doc:
        return []
    return [RoundOut(**{**round, "id": str(round["_id"])}) for round in doc["rounds"]]
...
async def get_game_round_by_id(game_id: str, round_id: str):
    doc = await app.mongodb["games"].find_one(
        {"_id": ObjectId(game_id), "rounds._id": ObjectId(round_id)}, {"rounds.$": 1}
    )
